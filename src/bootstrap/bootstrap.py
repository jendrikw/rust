...
def require(cmd, exit=True, exception=False):
    ...
    try:
        ...
        if exception:
            ...
            print(
                "error: unable to run `{}`: {}".format(" ".join(cmd), exc),
                file=sys.stderr,
            )
...
def default_build_triple(verbose):
    ...
    if platform_is_win32():
        try:
            ...
            if verbose:
                print(
                    "detected default triple {} from pre-installed rustc".format(
                        triple
                    ),
                    file=sys.stderr,
                )
            ...
            if verbose:
                print("pre-installed rustc not detected: {}".format(e), file=sys.stderr)
...
class RustBuild(object):
    ...
    def download_toolchain(self):
        ...
        if self.rustc().startswith(bin_root) and (
            not os.path.exists(self.rustc())
            or self.program_out_of_date(self.rustc_stamp(), key)
        ):
    ...
    def check_vendored_status(self):
        ...
        if "SUDO_USER" in os.environ and not self.use_vendored_sources:
            if os.getuid() == 0:
                ...
                print("      use vendored sources by default.", file=sys.stderr)
        ...
        if self.use_vendored_sources:
            ...
            if not os.path.exists(vendor_dir):
                ...
                print(
                    "       Run `cargo vendor {}` to initialize the "
            ...
            if not os.path.exists(cargo_dir):
                print(
                    "error: vendoring required, but .cargo/config does not exist.",
                    file=sys.stderr,
                )
...
def bootstrap(args):
    ...
    build.download_url = (
        os.getenv("RUSTUP_DIST_SERVER") or data["config"]["dist_server"]
    )
